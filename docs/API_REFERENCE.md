...
## Waiting (Currently Disabled)

The following waiting tools are currently commented out in `server.rb` but fully implemented and can be enabled by uncommenting lines 35-37.
...
| state | string | No | `visible`, `hidden`, or `exists` (default: visible) |
| session_id | string | Yes | Session ID to use |
...
```json
{
  "message": "Element hidden: .loading-spinner",
...
|------|------|----------|-------------|
| timeout | number | No | Maximum seconds to wait (default: 30) |
| wait_until | string | No | `load`, `domcontentloaded`, or `networkidle` (default: load) |
| session_id | string | Yes | Session ID to use |
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| seconds | number | Yes | Number of seconds to wait (0.1 - 60) |
| session_id | string | Yes | Session ID to use |
